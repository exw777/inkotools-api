...
При указании флага `--template` в качестве аргумента нужно передать имя файла шаблона из директории `templates`

#### Файлы шаблонов

Команда `send` поддерживает отправку списка команд из файла с возможностью использования шаблонов [jinja2](https://jinja.palletsprojects.com/). Внутри шаблонов можно использовать переменную `sw`, которая ссылается на экземпляр класса текущего коммутатора. Все файлы шаблонов хранятся в директории `templates`.
...
При запуске скрипта:
```shell
$ ./sw.py 59.75 send --template test.j2
```
